...
type RandomUaPipeline struct {
	goribot.Pipeline
	UaList  []string
	randSrc int64
}

func NewRandomUaPipeline() *RandomUaPipeline {
	return &RandomUaPipeline{}
}

func (s *RandomUaPipeline) OnRequest(spider *goribot.Spider, request *goribot.Request) *goribot.Request {
	if request.Header.Get("User-Agent") == "" {
		s.randSrc += 1
		rs := rand.NewSource(time.Now().Unix() + s.randSrc)
		ra := rand.New(rs)
		s.randSrc = ra.Int63()
		request.Header.Set("User-Agent", s.UaList[ra.Intn(len(s.UaList))])
	}
	return request
}

func (s *RandomUaPipeline) Init(spider *goribot.Spider) {
	s.UaList = []string{
